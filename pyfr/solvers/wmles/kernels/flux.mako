...
% endif
    ...
    fpdtype_t t_xx = -2*(mu_c*rcprho + nu_sgs)*(u_x - ${1.0/3.0}*(u_x + v_y));
    fpdtype_t t_yy = -2*(mu_c*rcprho + nu_sgs)*(v_y - ${1.0/3.0}*(u_x + v_y));
    fpdtype_t t_xy = -(mu_c*rcprho + nu_sgs)*(v_x + u_y);
...
% endif
    ...
    // Negated stress tensor elements
    fpdtype_t t_xx = -2*(mu_c*rcprho + nu_sgs)*(u_x - ${1.0/3.0}*(u_x + v_y + w_z));
    fpdtype_t t_yy = -2*(mu_c*rcprho + nu_sgs)*(v_y - ${1.0/3.0}*(u_x + v_y + w_z));
    fpdtype_t t_zz = -2*(mu_c*rcprho + nu_sgs)*(w_z - ${1.0/3.0}*(u_x + v_y + w_z));
    fpdtype_t t_xy = -(mu_c*rcprho + nu_sgs)*(v_x + u_y);
    fpdtype_t t_xz = -(mu_c*rcprho + nu_sgs)*(u_z + w_x);
    fpdtype_t t_yz = -(mu_c*rcprho + nu_sgs)*(w_y + v_z);
