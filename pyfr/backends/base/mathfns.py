...
from pyfr.dsl.nodes import Binary, Call, Float, Int, Region, Var, map_ast
from pyfr.dsl.simplifier import expand_power, expr_size
...
def lower_math_fns(ast, fns):
    helpers = set()

    def lower(node):
        # No region kind currently permits algebraic rearrangement
        ...
        match node:
            ...
            case Call(Var('pow'), [base, Float(v)]) if 'pow' in fns:
                lowered = _lower_pow(base, v, helpers)
                return lowered if lowered is not None else node
            case _:
                return node
