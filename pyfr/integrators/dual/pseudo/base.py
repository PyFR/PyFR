...
class BaseDualPseudoIntegrator(BaseCommon):
    ...
    def __init__(self, backend, systemcls, rallocs, mesh,
        ...
        if self._pseudo_norm not in {'l2', 'l4', 'l8', 'uniform', 'all'}:
            raise ValueError('Invalid pseudo-residual norm')

        if self._pseudo_norm == 'all':
            self._pseudo_residtol_l2 = residtol_l2 = []
            self._pseudo_residtol_l4 = residtol_l4 = []
            self._pseudo_residtol_l8 = residtol_l8 = []
            ...
            for v in elementscls.convarmap[self.system.ndims]:
                ...
                try:
                    residtol_l4.append(cfg.getfloat(sect, 'pseudo-resid-l4-tol-' + v))
                except NoOptionError:
                    residtol_l4.append(cfg.getfloat(sect, 'pseudo-resid-l4-tol'))

                try:
                    residtol_l8.append(cfg.getfloat(sect, 'pseudo-resid-l8-tol-' + v))
                except NoOptionError:
                    residtol_l8.append(cfg.getfloat(sect, 'pseudo-resid-l8-tol'))
