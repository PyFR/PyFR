...
class StdEulerStepper(BaseStdStepper):
    ...
    def step(self, t, dt):
        ...
        rhs(t, ut, f, rhs_has_been_called=self.rhs_has_been_called, post_processed=True)
...
class StdTVDRK3Stepper(BaseStdStepper):
    ...
    def step(self, t, dt):
        ...
        rhs(t, r0, r2, rhs_has_been_called=self.rhs_has_been_called, post_processed=True)
        self.rhs_has_been_called = True
        add(0.0, r1, 1.0, r0, dt, r2)

        # Second stage; r2 = -∇·f(r1); r1 = 0.75*r0 + 0.25*r1 + 0.25*dt*r2
        rhs(t + dt, r1, r2, rhs_has_been_called=self.rhs_has_been_called, post_processed=False)
        add(0.25, r1, 0.75, r0, 0.25*dt, r2)

        # Third stage; r2 = -∇·f(r1);
        #              r1 = 1.0/3.0*r0 + 2.0/3.0*r1 + 2.0/3.0*dt*r2
        rhs(t + 0.5*dt, r1, r2, rhs_has_been_called=self.rhs_has_been_called, post_processed=False)
...
class StdRK4Stepper(BaseStdStepper):
    ...
    def step(self, t, dt):
        ...
        # First stage; r1 = -∇·f(r0)
        rhs(t, r0, r1, rhs_has_been_called=self.rhs_has_been_called, post_processed=True)
        self.rhs_has_been_called = True

        # Second stage; r2 = r0 + dt/2*r1; r2 = -∇·f(r2)
        add(0.0, r2, 1.0, r0, dt/2.0, r1)
        rhs(t + dt/2.0, r2, r2, rhs_has_been_called=self.rhs_has_been_called, post_processed=False)
        ...
        # r2 = -∇·f(r2)
        add(dt, r2, 1.0, r0)
        rhs(t + dt, r2, r2, rhs_has_been_called=self.rhs_has_been_called, post_processed=False)
...
class StdRKVdH2RStepper(BaseStdStepper):
    ...
    def step(self, t, dt):
        run_kernels, rhs, postproc = self.backend.run_kernels, self.system.rhs, self.system.postproc
        ...
        for i, ci in enumerate(self.c):
            ...
            rhs(t + ci*dt, r2 if i > 0 else r1, r2,
                rhs_has_been_called=self.rhs_has_been_called, post_processed=post_processed)
            self.rhs_has_been_called = True
