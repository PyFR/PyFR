...
from pyfr.mpiutil import Scatterer, get_comm_rank_root
...
class NativeReader:
    def __init__(self, fname, partitioning=None, *, construct_con=True):
        self.f = h5py.File(fname, 'r')
        self.mesh = _Mesh(fname=fname)

        # Read in and transform the various parts of the mesh
        self._read_metadata()
        self._read_partitioning(partitioning)
        self._read_eles()
        self._read_nodes()
    ...
    def load_subset_mesh_soln(self, sname, prefix='soln'):
        ...
        with h5py.File(sname, 'r') as f:
            ...
            for et in self.escatter:
                ...
                if f'{prefix}/{et}' not in f:
                    subset[et] = []
                    continue
                # If the element is partially subset use a sparse scatterer
                elif f'{prefix}/{et}_idx' in f:
                    pass
                # Complete element present so reuse the eles scatterer
                else:
                    escatter = self.escatter[et]

                # Read the solution
                esoln = escatter(f[f'{prefix}/{et}'])
                if len(esoln):
                    soln[f'{prefix}_{et}'] = esoln.swapaxes(0, 2)

                # Read the partition data
                epart = escatter(f[f'{prefix}/{et}_parts'])
                if len(epart):
        ...
        if subset:
            ...
            for etype in self.mesh.spts:
                if etype in subset:
                    ...
                    if len(sidx):
                        eidxs[etype] = sidx
                        spts[etype] = self.mesh.spts[sidx]
            ...
            mesh = replace(self.mesh, eidxs=eidxs, spts=spts,
    ...
    def _select_partitioning(self, size, pname=None):
        ...
        if pname:
            ...
            if nparts != size:
                raise RuntimeError(f'Partitioning {pname} has {nparts} but '
                                   f'running with {size} ranks')
