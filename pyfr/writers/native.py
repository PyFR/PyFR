...
class NativeWriter:
    ...
    def set_shapes_eidxs(self, shapes, eidxs):
        ...
        for etype, ecount in self._ecounts.items():
            ...
            if any(eshape):
                ...
                self._einfo[etype] = (gatherer, subset, shape, noff)
    ...
    def write(self, data, tcurr, metadata=None):
        ...
        self._write(path, gdata, metadata)
    ...
    def _write_parallel(self, path, data, metadata):
        ...
        # Create the dataset info array
        dinfo = []
        for etype, (gatherer, subset, shape, *_) in self._einfo.items():
            dinfo.append((f'{prefix}/{etype}', shape, self.fpdtype))
            dinfo.append((f'{prefix}/{etype}_parts', shape[0:1], np.int32))

            if subset:
                dinfo.append((f'{prefix}/{etype}_idxs', shape[0:1], int))

        # Add the metadata records
        if rank == root:
            minfo = [(k, v.shape, v.dtype.str) for k, v in metadata.items()]
        else:
            minfo = None

        # Broadcast this metadata information
        dinfo += comm.bcast(minfo, root=root)

        with h5py.File(path, 'w', driver='mpio', comm=comm) as f:
            # Collectively create all of the datasets
            for name, shape, dtype in dinfo:
            ...
            # If we are the root rank then also write out the metadata
            if rank == root:
                for k, v in metadata.items():
                    f[k].write_direct(v)
    ...
    def _write_serial(self, path, data, metadata):
        ...
        if rank != root:
            ...
            with h5py.File(path, 'w') as f:
                ...
                for etype, (gatherer, subset, shape, noff) in einfo.items():
                    dtype = self.fpdtype
