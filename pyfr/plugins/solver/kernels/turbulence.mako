...
<%pyfr:macro name='turbulence' params='t, u, ploc, src' externs='tinit, state'>
  ...
  % for i in range(nvmax):
    ...
    g = (delta2[0] < ${ls**2} && delta2[1] < ${ls**2} && delta2[2] < ${ls**2} && tpos[0] <= ${ls} && state[${i}] > 0) ? exp(${beta1}*arg) : 0.0;
  ...
  clip = (tploc[0] < ${ls} && tploc[0] > ${-ls}) ? ${beta3*avgu/ls}*exp(${-0.5*np.pi/ls**2}*tploc[0]*tploc[0]) : 0.0;

    src[0] += ${beta2}*utilde[0]*clip;
    src[${nvars - 1}] += ${0.5*beta3}*u[0]*(${pyfr.dot('utilde[{i}]', i=3)})*clip;

  % for i in range(3):
    src[${i + 1}] += utilde[${i}]*u[0];
  % endfor
</%pyfr:macro>
