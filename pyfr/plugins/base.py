...
class RegionMixin(object):
    def _init_writer_for_region(self, intg, nout, prefix):
        ...
        # All elements on a boundary
            ...
            mdata = self._prepare_mdata_bcs(intg, nout, prefix, region)
            self._add_region_data = self._add_region_data_subset

        # Construct the file writer
        return NativeWriter(intg, mdata, basedir, basename)

    def _prepare_mdata_all(self, intg, nout, prefix):
        ...
        # Output metadata
        return [(f'{prefix}_{etype}', (nupts, nout, neles), dtype)
                for etype, (nupts, nvars, neles) in einfo]

    def _prepare_mdata_box(self, intg, nout, prefix, x0, x1):
        eset = {}
        ...
        return self._prepare_eset(intg, nout, prefix, eset)

    def _prepare_mdata_bcs(self, intg, nout, prefix, bcname):
        comm, rank, root = get_comm_rank_root()
    ...
    def _prepare_eset(self, intg, nout, prefix, eset):
        ...
        for etype, eidxs in sorted(eset.items()):
            ...
            mdata.append((f'{prefix}_{etype}', shape, intg.backend.fpdtype))
